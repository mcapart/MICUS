...
from analisys import calculate_time_between_peaks
from graphs import plot_derivative_avg_ears_with_peaks
...
def iterate_files(folder_name):
    ...
    for file_name in os.listdir(folder_name):
        ...
        if os.path.isfile(file_path):
            ...
            if time_between_peaks is not None and len(pairs) > 1:
                ...
                if avg != -1:
                    averages.append(avg)
            time_between_peaks_mediapipe, pairs_mediapipe = calculate_time_between_peaks(left_eye_ears_mediapipe, right_eye_ears_mediapipe, time_stamps)
            if time_between_peaks_mediapipe is not None and len(pairs_mediapipe) > 1:
            ...
            dlib_blinks = len(pairs)
            blinks_per_minute.append(60*dlib_blinks/time_stamps[-1])
            mediapipe_blinks = len(pairs_mediapipe)
            blinks_per_minute_mediapipe.append(60 * mediapipe_blinks / time_stamps[-1])
            if mediapipe_blinks > dlib_blinks:
                dlib += 1



    print(f"results dlib: {np.array(averages).mean()}")
    # print(f"results wo angry: {np.array(averages_wo_an).mean()}")
    # print(f"results angry: {np.array(angry_average).mean()}")
    #print(averages_low)
    print('-----')
    print(f"results mediapipe: {np.array(averages2).mean()}")
    # print(f"results wo angry: {np.array(averages_wo_an2).mean()}")
    # print(f"results angry: {np.array(angry_average2).mean()}")
    #print(averages_low2)
    print("---")
    print(total, dlib)
    print(f"blinks per minute dlib: {np.array(blinks_per_minute).mean()}")
    print(f"blinks per minute mediapipe: {np.array(blinks_per_minute_mediapipe).mean()}")
#
...
def analyze_video(file_path):
    if os.path.isfile(file_path):
        ...
        #     avg = np.mean(np.diff(time_between_peaks))
        #     print(f"dlib avg {avg}, {len(pairs)}", time_between_peaks)
        ...
        #     avg2 = np.mean(np.diff(time_between_peaks_mediapipe))
        #     print(f"mediapipe avg {avg2}, {len(pairs_media_pipe)}", time_between_peaks_mediapipe)
        ...
        return plot_derivative_avg_ears_with_peaks(left_eye_ears_mediapipe, right_eye_ears_mediapipe, time_stamps, "mediapipe")
        # plot_avg_ears_by_time(left_eye_ears_mediapipe, right_eye_ears_mediapipe, time_stamps, "mediapipe")
        #plot_avg_ears_with_peaks(left_eye_ears, right_eye_ears, time_stamps)
        #plot_avg_ears_with_peaks(left_eye_ears, right_eye_ears, time_stamps, "mediapipe")
...
if __name__ == '__main__':
    ...
    #analyze_video("/Users/micacapart/Documents/ITBA/pf-2023b-deepfake-detection/DS/eyeblink8/9/27122013_152435_cam.avi")
    analyze_video("/validation/RES-26122013_223310_cam")
